...
const JobPage = ({handelJobDeleteRequest}) => {
    const { id } = useParams();
    const jobData = useLoaderData();
    const navigate = useNavigate();

    const onDeleteBtnClick = (jobId) => {
        const confirm = window.confirm(`Are you sure you want to delete this Job listing having Job Id: ${jobId}`);

        if (!confirm) return

        handelJobDeleteRequest(jobId);
          ...
          <aside>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md mt-6">
              ...
              <Link
                ...
                >Edit Job</Link
              >
              <button onClick={() => {onDeleteBtnClick(jobData.id)}}
